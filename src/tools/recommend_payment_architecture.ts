...
import { recommendPaymentArchitecture } from "../advisors/architecture.js";

export function registerRecommendPaymentArchitecture(server: McpServer) {
  server.tool(
    "recommend_payment_architecture",
    "Recommend a payment architecture based on business, geography, and operating model.",
    {
    ...
    async ({
      ...
      const recommendation = recommendPaymentArchitecture({
      ...
      return text(`## Payment Architecture Recommendation

| Area | Recommendation |
| --- | --- |
| Recommended architecture | ${recommendation.recommendedArchitecture} |
...
### Educational note
This is a deterministic and educational architecture recommendation only.`);
    }
  );
}
