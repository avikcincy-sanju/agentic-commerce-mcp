...
import { recommendCrossborderModel } from "../advisors/architecture.js";

export function registerRecommendCrossborderModel(server: McpServer) {
  server.tool(
    "recommend_crossborder_model",
    "Recommend a cross-border payment architecture for a given market context.",
    {
      ...
      cross_border_requirements: z.string().default("Yes"),
    ...
    async ({ business_model, merchant_ownership, country, region, settlement_preference, scale, gmv, transaction_volume, cross_border_requirements, marketplace, isv, platform, pay_fac, embedded_finance, stablecoin_preference }) => {
      const recommendation = recommendCrossborderModel({
      ...
      return text(`## Cross-Border Model Recommendation

| Area | Recommendation |
| --- | --- |
| Cross-border architecture | ${recommendation.recommendedArchitecture} |
...
### Educational note
This is a deterministic cross-border architecture recommendation.`);
    }
  );
}
