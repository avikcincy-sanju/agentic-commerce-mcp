...
import { recommendAgenticArchitecture } from "../advisors/architecture.js";

export function registerRecommendAgenticArchitecture(server: McpServer) {
  server.tool(
    "recommend_agentic_architecture",
    "Recommend an agentic commerce architecture for a given business context.",
    {
    ...
    async ({ business_model, merchant_ownership, country, region, settlement_preference, scale, gmv, transaction_volume, cross_border_requirements, marketplace, isv, platform, pay_fac, embedded_finance, stablecoin_preference }) => {
      const recommendation = recommendAgenticArchitecture({
      ...
      return text(`## Agentic Architecture Recommendation

| Area | Recommendation |
| --- | --- |
| Agentic architecture | ${recommendation.recommendedArchitecture} |
...
### Educational note
This is a deterministic agentic architecture recommendation.`);
    }
  );
}
